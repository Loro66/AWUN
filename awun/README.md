...
AWUN is a FastAPI music-search aggregator. It searches YouTube and SoundCloud through `yt-dlp`, optionally searches VK through the official API, and returns normalized metadata plus ephemeral direct audio URLs.
...
`render.yaml` Blueprint is included for Render; add `AWUN_VK_ACCESS_TOKEN` in
the host dashboard if VK search is required.
...
Copy `.env.example` to `.env`. YouTube and SoundCloud work without API keys. VK is added only when `AWUN_VK_ACCESS_TOKEN` is set. VK controls audio API access and may not grant it to every application or token.
