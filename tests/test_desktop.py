...
def test_windows_builds_embed_the_songvale_icon() -> None:
    ...
    icon = ROOT / "desktop" / "assets" / "songvale.ico"
    installer = (ROOT / "installer" / "AWUN.iss").read_text(encoding="utf-8")

    assert "AWUN.spec" in batch
    assert "AWUN.spec" in workflow
    assert '"desktop" / "assets" / "songvale.ico"' in spec
    assert 'name="SONGVALE"' in spec
    assert '"frontend"' in spec
    assert '"VERSION"' in spec
    assert icon.read_bytes().startswith(b"\x00\x00\x01\x00")
    assert icon.stat().st_size > 10_000
    assert "SONGVALE-Setup-x64" in workflow and "SONGVALE-Setup-x64" in installer
    assert "SONGVALE.exe" in workflow and "SONGVALE.exe" in installer
...
def test_desktop_uses_local_backend_before_the_free_remote_fallback() -> None:
    ...
    assert "SONGVALE_REMOTE_API_URL" in launcher and "AWUN_REMOTE_API_URL" in launcher
...
def test_desktop_reads_legacy_state_until_songvale_state_exists(tmp_path: Path, monkeypatch) -> None:
    sys.modules.setdefault("webview", SimpleNamespace())
    from desktop.launcher import DesktopStateBridge

    monkeypatch.setenv("APPDATA", str(tmp_path))
    legacy = tmp_path / "AWUN" / "desktop-state.json"
    legacy.parent.mkdir()
    legacy.write_text('{"awun-library":"[]"}', encoding="utf-8")
    bridge = DesktopStateBridge()

    assert json.loads(bridge.load_state()) == {"awun-library": "[]"}
    assert bridge.save_state('{"awun-language":"ru"}') is True
    assert json.loads(bridge.load_state()) == {"awun-language": "ru"}
    assert (tmp_path / "SONGVALE" / "desktop-state.json").exists()
