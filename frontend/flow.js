(() => {
  ...
  const PROFILE_KEY='awun-flow-profile-v1';
  ...
  const ids=['flowButton','flowBadge','flowPanel','flowClose','flowStart','flowSeed','flowStats','flowReset','flowMood','flowActivity','flowDiscovery','flowLike','flowDislike'];
  const flow=Object.fromEntries(ids.map(id=>[id,document.getElementById(id)]));
  if(ids.some(id=>!flow[id]))return;

  function freshProfile(){return{version:1,discovery:'balanced',mood:'any',activity:'any',signals:[]}}
  function loadProfile(){
    try{
      const value=JSON.parse(localStorage.getItem(PROFILE_KEY)||'null');if(!value||!Array.isArray(value.signals))return freshProfile();
      return{version:1,discovery:['familiar','balanced','new'].includes(value.discovery)?value.discovery:'balanced',mood:moodTerms[value.mood]!==undefined?value.mood:'any',activity:activityTerms[value.activity]!==undefined?value.activity:'any',signals:value.signals.slice(-800)};
  ...
  function rankCandidates(candidates){
    const blocked=dislikedIds(),unique=new Map();candidates.forEach(track=>{if(track?.id&&!blocked.has(track.id)&&track.source!=='yandex_music'&&!unique.has(track.id))unique.set(track.id,track)});
  ...
  function contextTerms(){return[moodTerms[profile.mood],activityTerms[profile.activity]].filter(Boolean).join(' ')}
  ...
  async function fillFlow(initial=false){
    ...
    try{
      ...
      if(initial&&!state.active)await playTrack(state.tracks[0]);setMessage(`FLOW is live · ${state.tracks.length} ranked tracks · feedback changes the next refill.`,'notice');
    }catch(error){setMessage(error.message||'Flow is temporarily unavailable.','error');if(initial)stopFlow()}
  ...
  function stopFlow(){state.flow.active=false;document.body.classList.remove('flow-active');updateFlowUi();setMessage('FLOW stopped. Your taste profile is saved on this device.','notice')}
  ...
  function updateFlowUi(){
    flow.flowBadge.textContent=state.flow.active?'LIVE':state.flow.fetching?'BUILDING':'READY';flow.flowButton.classList.toggle('active',state.flow.active);flow.flowStart.textContent=state.flow.active?'STOP FLOW':'START FLOW ↗';flow.flowStart.classList.toggle('stop',state.flow.active);flow.flowStats.textContent=`${profile.signals.length} signal${profile.signals.length===1?'':'s'} learned · local only`;
    flow.flowMood.value=profile.mood;flow.flowActivity.value=profile.activity;flow.flowDiscovery.querySelectorAll('[data-flow-discovery]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.flowDiscovery===profile.discovery)));
  ...
  flow.flowButton.addEventListener('click',()=>flow.flowPanel.hidden?openPanel():closePanel());flow.flowClose.addEventListener('click',closePanel);flow.flowStart.addEventListener('click',startFlow);flow.flowLike.addEventListener('click',likeCurrent);flow.flowDislike.addEventListener('click',dislikeCurrent);
