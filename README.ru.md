![SONGVALE — один поиск, вся музыка](docs/awun-github-banner.svg)
...
[![Скачать для Windows](https://img.shields.io/badge/DOWNLOAD_WINDOWS-F3F2E9?style=for-the-badge&logo=windows&logoColor=11120F)](https://github.com/Loro66/AWUN/releases/latest/download/SONGVALE-Setup-x64.exe)
...
SONGVALE рассматривает такие сбои как штатное состояние системы.

| Проблема | Решение SONGVALE |
| --- | --- |
...
Windows-версия упаковывает frontend и FastAPI backend в один EXE, запускает API на случайном локальном порту и сначала выполняет поиск локально. Публичный backend SONGVALE используется только как резерв для отдельного недоступного источника.
...
Скачай [установщик для текущего пользователя](https://github.com/Loro66/AWUN/releases/latest/download/SONGVALE-Setup-x64.exe) или [portable EXE](https://github.com/Loro66/AWUN/releases/latest/download/SONGVALE.exe). Рядом с каждым файлом в релизе опубликован SHA-256.
...
Релиз `v2.4.0` автоматически собирается из `main` через GitHub Actions.
...
- **33 Playwright-сценария** проверяют постепенный поиск, отмену, тайм-аут источника, восстановление воспроизведения, возобновляемый перенос медиатеки, ручную проверку совпадений, звуковые профили, очередь, большие библиотеки и адаптивность.
...
| Windows | Выпущенная неподписанная бета | [Скачать v2.4.0](https://github.com/Loro66/AWUN/releases/tag/v2.4.0) |
...
Публичная доступность не означает подтверждённую аудиторию. SONGVALE пока не заявляет количество пользователей, retention или выручку. В [проверяемой справке о проекте](docs/PROJECT_CASE_STUDY.ru.md) реализованные результаты отделены от будущего пользовательского пилота.
...
SONGVALE является **проприетарным freeware-проектом с доступным для просмотра кодом**, а не open-source программой. Официальные неизменённые сборки можно бесплатно использовать на условиях [лицензии SONGVALE 1.0](LICENSE.md) и [EULA](EULA.md). Повторное использование кода и публикация изменённых сборок требуют письменного разрешения.
