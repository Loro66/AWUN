...
Для сети, где провайдер блокирует отдельные источники, Windows-версия поддерживает
явный удалённый endpoint. Укажи `AWUN_REMOTE_API_URL=https://твой-домен` перед
запуском или запиши HTTPS-адрес одной строкой в
`%APPDATA%\\AWUN\\remote-api.txt`. Тогда поиск, тексты, импорт и AWUN-медиа
идут через твой mirror или свой сервер с AWUN API, без системного VPN. Не
используй неизвестные IP из чужих hosts-файлов: это прокси неизвестного
владельца и они могут перехватывать трафик или внезапно перестать работать.
