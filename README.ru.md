...
Опубликованные бета-сборки v2.5.1 не подписаны Authenticode-сертификатом, поэтому Windows SmartScreen может показать предупреждение. Workflow релиза теперь подписывает и проверяет будущие сборки, когда в репозитории настроен сертификат; контрольную сумму всё равно нужно проверять.
...
- Windows workflow создаёт два EXE, при наличии сертификата подписывает и проверяет их, рассчитывает SHA-256 и публикует GitHub Release только после успешных тестов.
